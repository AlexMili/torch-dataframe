...
function Dataframe:init_batch(...)
	local args = dok.unpack(
		...
		{arg='data_types', type='table',
		 help='Types of data with corresponding proportions to split to.',
		 default={['train'] = 0.7,
							['validate'] = 0.2,
							['test'] = 0.1}},
	...
	if (self.batch == nil or self.batch.shuffle ~= args.shuffle) then
		self.batch = {
		...
		for k,p in pairs(args.data_types) do
			if (self.batch.data_types[k] == nil or self.batch.data_types[k] ~= p) then
				new_types = true
				break
			end
		end
		...
		if (not new_types) then
			for k,p in pairs(self.batch.data_types) do
				if (args.data_types[k] == nil or args.data_types[k] ~= p) then
					new_types = true
					break
				end
			end
